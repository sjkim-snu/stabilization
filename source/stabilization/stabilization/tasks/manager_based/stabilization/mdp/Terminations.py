...
def _is_stable_from_obs(
    ...
    lin_vel_w = mdp.ObservationFns.get_lin_vel_w(env, asset_cfg)          # (N, 3)
    within_lin = torch.linalg.norm(lin_vel_w, dim=-1) <= lin_vel_tol_mps  # (N,)

    # determine whether angular velocity is within tolerance
    ang_vel_b = mdp.ObservationFns.get_ang_vel_b(env, asset_cfg)           # (N, 3)
    within_ang = torch.linalg.norm(ang_vel_b, dim=-1) <= ang_vel_tol_radps # (N,)

    # determine whether roll and pitch is within tolerance
...
class TerminationFns:
    ...
    def is_flipped(
        ...
        # determine if roll or pitch exceeds the tilt threshold
    ...
    def is_nan_or_inf(
        ...
        quat = mdp.ObservationFns.get_quaternion_w(env, asset_cfg)             # (N, 4)
        roll, pitch, yaw = math_utils.euler_xyz_from_quat(quat)                # each (N,)
        ...
        mats = torch.cat([pos_err_w, lin_vel_w, ang_vel_b], dim=-1)            # (N, 9)
